                        ...
                        Row(
                            ...
                            Text(
                                ...
                                onTextLayout = { textLayout ->
                                    ...
                                    val annotation = textAnnotation.getStringAnnotations("태그", 0, textAnnotation.lastIndex).let {
                                        it.forEach { list ->
                                    ...
                                    onDraw = {
                                        for (bound in textBounds) {
                                            val underline = bound.copy(top = bound.bottom * 0.93f) // top 을 bottom 에서 일정 비율만큼 빼도록 하기 위한 값 설정, bottom - top 이 size 에서 height 값이 된다.

                                            // span style
                                            val spanStyle = textAnnotation.spanStyles.first { range -> maxOf(annotation.start, range.start) < minOf(annotation.end, range.end) }
                                            "first: ${spanStyle.item.fontSize}".d("checkSize")

                                            "bound: $bound, underlineBound: $underline".d("checkSize")
                                            "offset (top) : ${bound.topLeft} , ${bound.topCenter} , ${bound.topRight}".d("checkSize")
                                            "offset (center) : ${bound.centerLeft} , ${bound.center} , ${bound.centerRight}".d("checkSize")
                                            "offset (bottom) : ${bound.bottomLeft} , ${bound.bottomCenter} , ${bound.bottomRight}".d("checkSize")
